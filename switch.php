...
<div align="center">
	...
	<a href="switch_detail.php" class="btn btn-primary btn-lg">ADD</a>
	</div>
...
$query = "SELECT count(*) FROM switch";
...
$query = "SELECT switch.asset_num, mgmt_num, reg_date, location, spec, asset_name, standard, service_life FROM asset, switch where asset.asset_num = switch.asset_num LIMIT $limit_idx, $page_list";
$stmt = $conn->prepare($query);
$stmt->execute();
$today = date("Y-m-d");
$result = $stmt->fetch(PDO::FETCH_NUM);
...
function deleteConfirm(x, y){
	...
	if(r == true) {
		location.replace("del_asset.php?category=switch&asset_num=" + x + "&mgmt_num=" + y);
	} 
}
</script>
</body></html>
