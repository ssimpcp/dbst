...
<td style="width:10%"><input style="width:100%" type="submit" class="btn btn-default submit" name = "submit" id="search-box"value = "검색"/></td>
	...
	<?php
	  ...
	  $block_set = 10;
	  if(!$keyword){
	    $query = "SELECT count(*) FROM server NATURAL JOIN asset";
	  }else{
	    if(is_numeric($keyword)){
	      $query = "SELECT count(*) FROM server as A NATURAL JOIN asset as B WHERE A.asset_num LIKE '%$keyword%' || A.mgmt_num LIKE '%$keyword%' || reg_date LIKE '%$keyword%' || location LIKE '%$keyword%' || spec LIKE '%$keyword%' || standard LIKE '%$keyword%' || core LIKE '%$keyword%' || asset_name = '%$keyword%'";
	    }else{
	      $query = "SELECT count(*) FROM server as A NATURAL JOIN asset as B WHERE A.mgmt_num LIKE '%$keyword%' || location LIKE '%$keyword%' || spec LIKE '%$keyword%' || asset_name LIKE '%$keyword%' || standard LIKE '%$keyword%'";
	    }
	  }
	  ...
	  if(!$keyword){
	    ...
	    if(is_numeric($keyword)){
	      $query = "SELECT A.asset_num, mgmt_num, reg_date, location, spec, core, asset_name, standard, service_life FROM server as A NATURAL JOIN asset as B WHERE A.asset_num LIKE '%$keyword%' || A.mgmt_num LIKE '%$keyword%' || B.reg_date LIKE '%$keyword%' || B.location LIKE '%$keyword%' || A.spec LIKE '%$keyword%' || B.standard LIKE '%$keyword%' || A.core = '%$keyword%' || A.asset_name = '%$keyword%' LIMIT $limit_idx, $page_list";
	      echo $query;
	    }else{
	      $query = "SELECT A.asset_num, mgmt_num, reg_date, location, spec, core, asset_name, standard, service_life FROM server as A NATURAL JOIN asset as B WHERE A.mgmt_num LIKE '%$keyword%' || location LIKE '%$keyword%' || spec LIKE '%$keyword%' || asset_name LIKE '%$keyword%' || standard LIKE '%$keyword%' LIMIT $limit_idx, $page_list";
	    }
	  }
	  $stmt = $conn->prepare($query);
	  $stmt->execute();
